...
// Persisted Dashboard State (for localStorage)
export interface PersistedDashboardState {
  step0Status: StepState;
  productCount: number;
  step1Status: StepState;
  step2Status: StepState;
  step3Status: StepState;
  step4Status: StepState;
  categories: Category[];
  step4Products: ProductsByCategoryEndpoint[];
  scrapingJobs: { [jobId: string]: ScrapeJob };
  minimizedJobs: string[]; // Set converted to array for serialization
  showStep4Dropdown: boolean;
