 ...
 */
const AustliftScraperDashboard: React.FC = () => {
  const [activeSection, setActiveSection] = useState<string>('home');
  const [categories, setCategories] = useState<Category[]>([]);
  const [selectedCategories, setSelectedCategories] = useState<Set<string>>(new Set());
  const [step1Status, setStep1Status] = useState<StepState>({ status: 'idle', message: '' });
  const [step2Status, setStep2Status] = useState<StepState>({ status: 'idle', message: '' });
  const [step3Status, setStep3Status] = useState<StepState>({ status: 'idle', message: '' });
  const [step4Status, setStep4Status] = useState<StepState>({ status: 'idle', message: '' });
   ...
   * @returns Promise that resolves when categories are refreshed
   * @throws {Error} When API request fails
   */
  ...
  const handleStep1 = async (): Promise<void> => {
    ...
    setStep1Status({ status: 'success', message: '✅ Step 1 Complete! Categories loaded successfully' });
  };
  ...
  const handleStep2 = async (): Promise<void> => {
    ...
    setStep2Status({ status: 'loading', message: 'Selecting all categories...' });
  ...
  const handleStep3 = async (): Promise<void> => {
    setStep3Status({ status: 'loading', message: 'Initializing previous steps...' });
    await handleStep2();
    ...
    try {
      ...
      const data = await response.json();
      setStep3Status({ status: 'success', message: `✅ Complete! Loaded ${data.total || 0} products` });
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : 'Failed to load products';
      setStep3Status({ status: 'error', message: `❌ Error loading products: ${errorMessage}` });
      setError({ hasError: true, message: errorMessage });
  ...
  const handleStep4 = async (): Promise<void> => {
    setStep4Status({ status: 'loading', message: 'Initializing previous steps...' });
    await handleStep3();

    setStep4Status({ status: 'loading', message: 'Loading ALL products for table display...' });
    try {
      ...
      if (!data.products || data.products.length === 0) {
        setStep4Products([]);
        setStep4Status({ status: 'error', message: '⚠️ No products available for display' });
        return;
      }
      ...
      const errorMessage = err instanceof Error ? err.message : 'Failed to load products';
      setStep4Status({ status: 'error', message: `❌ Error loading products: ${errorMessage}` });
      setStep4Products([]);
      setError({ hasError: true, message: errorMessage });
      console.error('Error loading products:', err);
    }
                  ...
                  <div className='flex items-center justify-between p-4 border border-gray-200 rounded-lg'>
                    <div className='flex items-center space-x-3'>
                      ...
                      <div>
                        ...
                        {step1Status.message && (
                          <p className={`text-sm ${
                            step1Status.status === 'success' ? 'text-green-600' :
                            step1Status.status === 'error' ? 'text-red-600' :
                            'text-blue-600'
                          }`}>{step1Status.message}</p>
                        )}
                      </div>
                    </div>
                    <button
                  ...
                  <div className='flex items-center justify-between p-4 border border-gray-200 rounded-lg'>
                    <div className='flex items-center space-x-3'>
                      ...
                      <div>
                        ...
                        {step2Status.message && (
                          <p className={`text-sm ${
                            step2Status.status === 'success' ? 'text-green-600' :
                            step2Status.status === 'error' ? 'text-red-600' :
                            'text-green-600'
                          }`}>
                            {step2Status.message}
                          </p>
                  ...
                  <div className='flex items-center justify-between p-4 border border-gray-200 rounded-lg'>
                    <div className='flex items-center space-x-3'>
                      ...
                      <div>
                        ...
                        {step3Status.message && (
                          <p className={`text-sm ${
                            step3Status.status === 'success' ? 'text-green-600' :
                            step3Status.status === 'error' ? 'text-red-600' :
                            'text-purple-600'
                          }`}>
                            {step3Status.message}
                          </p>
                  ...
                  <div className='flex items-center justify-between p-4 border border-gray-200 rounded-lg'>
                    <div className='flex items-center space-x-3'>
                      ...
                      <div>
                        ...
                        {step4Status.message && (
                          <p className={`text-sm ${
                            step4Status.status === 'success' ? 'text-green-600' :
                            step4Status.status === 'error' ? 'text-red-600' :
                            'text-orange-600'
                          }`}>
                            {step4Status.message}
                          </p>
