...
const AustliftScraperDashboard = () => {
  ...
  const fetchProducts = useCallback(async (limit = 20, offset = 0) => {
    try {
      const response = await fetch(`/products?limit=${Number(limit)}&offset=${Number(offset)}`);
      if (!response.ok) {
        throw new Error(`HTTP ${response.status}: ${response.statusText}`);
      }
      const data = await response.json();
  ...
  const handleStep2 = async () => {
    ...
    try {
      ...
      const data = await response.json();
      setStep2Status(`✅ Complete! Steps 1 & 2 finished successfully! Job ID: ${data.job_id}`);
    } catch (err) {
  ...
  const handleStep4 = async () => {
    ...
    try {
      ...
      setStep4Status(`✅ CSV generated! ${data.products.length} products exported`);
                ...
                <div className='space-y-4'>
                  ...
                  <div className='flex items-center justify-between p-4 border border-gray-200 rounded-lg'>
                    <div className='flex items-center space-x-3'>
                      ...
                      <div>
                        ...
                        {step1Status && <p className='text-sm text-blue-600'>{step1Status}</p>}
                      </div>
                    </div>
                    <button
                      type='button'
                  ...
                  <div className='flex items-center justify-between p-4 border border-gray-200 rounded-lg'>
                    <div className='flex items-center space-x-3'>
                      ...
                      <div>
                        <h4 className='font-medium text-gray-900'>Scrape Product Lists</h4>
                        <p className='text-sm text-gray-600'>Start scraping all products from selected categories</p>
                        {step2Status && <p className='text-sm text-green-600'>{step2Status}</p>}
                      </div>
                    </div>
                    <button
                      type='button'
                  ...
                  <div className='flex items-center justify-between p-4 border border-gray-200 rounded-lg'>
                    <div className='flex items-center space-x-3'>
                      ...
                      <div>
                        <h4 className='font-medium text-gray-900'>Review Scraped Products</h4>
                        <p className='text-sm text-gray-600'>Review and validate scraped product data</p>
                        {step3Status && <p className='text-sm text-purple-600'>{step3Status}</p>}
                      </div>
                    </div>
                    <button
                      type='button'
                  ...
                  <div className='flex items-center justify-between p-4 border border-gray-200 rounded-lg'>
                    <div className='flex items-center space-x-3'>
                      ...
                      <div>
                        ...
                        <p className='text-sm text-gray-600'>Export all products to CSV format</p>
                        {step4Status && <p className='text-sm text-orange-600'>{step4Status}</p>}
                      </div>
                    </div>
                    <button
                      type='button'
